...
7. Creating and editing a private study memo
8. Viewing iCloud sync, JSON backup and restore, and privacy information in Settings
...
10. Open “학습 메모” (Study Memos) and tap the plus button to create a private study page.
11. Settings are available from the gear button on the Days screen. Settings include iCloud information, JSON backup and restore, optional JSON word import, privacy information, and a button to replay the quick-start guide.
...
The app does not use third-party authentication, payment processors, advertising SDKs, analytics SDKs, tracking SDKs, remote AI APIs, or third-party data providers.
...
MAIN TEST FLOW: Launch the app → complete or skip the four-step quick-start guide → open 추가 (Add) → select 데이 0 or create a Day → enter `reservation` → tap AI로 생성 (Generate with AI) on a supported Apple Intelligence device, or press Return for manual entry → review/edit the pending content → tap the bottom save button → open 데이 (Days) and the saved Day → tap the English word for pronunciation and its number for details → open 복습 (Review) → open Today’s Review → reveal meanings → mark every word 다시 (Again) or 알아요 (Known) → complete the review → open 학습 메모 (Study Memos) to create a private note. Settings are available through the gear button on the Days screen.
...
CORE APPLE SERVICES: SwiftData local storage; the user’s private iCloud CloudKit database for sync; Apple Foundation Models for optional on-device generation; AVSpeechSynthesizer for pronunciation; system document pickers for user-initiated JSON backup/restore and Markdown import/export. There are no remote AI APIs, third-party authentication services, payments, ads, analytics, tracking, or third-party data providers. Optional external-AI JSON use is entirely manual copy/paste and is not an integration.

PERMISSIONS: The app does not request location, contacts, camera, microphone, photos, or tracking access. Text-to-speech does not use the microphone. File and paste access occur only after an explicit user action.
...
- Complete the review and show that only Again words remain due today.

### 2:25–2:45 — Study memo

- Open Study Memos.
- Create a new page, enter a title and a short note, and return to the list.

### 2:45–3:10 — Settings and privacy
