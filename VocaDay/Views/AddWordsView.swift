...
struct AddWordsHelpView: View {
    ...
    - 모든 값은 문자열로 작성해줘.
    - meaningKo에는 품사 약어를 붙인 자연스러운 한국어 뜻을 넣어줘. 자주 쓰는 품사만 최대 2개, 흔한 품사부터. 예: "v. 연기하다, 미루다" / "n. 기록, 음반 / v. 기록하다, 녹음하다"
    - exampleEn에는 자연스러운 영어 예문을 넣어줘. 반드시 english 단어 자체(또는 -s, -ed, -ing 같은 활용형)를 그대로 한 번 포함해야 해. 동의어나 대명사로 바꾸지 마. 이 단어 자리가 빈칸 문제의 정답이 돼.
    - 품사가 1개면 번호 없이 한 문장, 2개면 "1. 문장\\n2. 문장"처럼 meaningKo의 품사 순서대로 그 품사로 쓰인 문장을 하나씩 넣어줘.
    - exampleKo에는 영어 예문의 자연스러운 한국어 번역을 반드시 넣어줘. 예문이 2개면 같은 번호로 "1. 번역\\n2. 번역". 빈칸 문제의 힌트로 쓰여.
